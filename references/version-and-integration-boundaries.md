...
| `dsh.client.external` | 精确模块 specifier | 非基线同步 `require` 的供应与代码到达顺序 | Cordis 服务激活 |
...
baseline 按完整版本选择：`0.1.2-rc.1` 已移除 runtime client，平台表提供 React、React DOM、Cordis、`client-store`、`ui-slots` 和 `ui-primitives`。`0.1.5-alpha.1` 额外提供 `ui-dockkit`。不要把这些名字重复写进 `dsh.client.external`。非基线值导入才写 `external`；类型导入会被擦除，不形成模块请求。
...
Desktop 等启动器可能重新启用插件已禁用的官方 Loader 行。记录实际应用版本与模式，并检查最终 generation、boot manifest 或 launcher 诊断，不能把某个旧版本的覆盖规则永久套用。
...
- **旧版 Settings 结论外推**：旧版的 namespace 白名单结论不能外推；以目标 Controller 与实际 `settings.describe` 为准。
- **`inject` 与 `external` 混用**：前者不排序代码，后者不提供 Cordis 服务。把值模块只写进 `inject` 会在当前版本中留下同步 `require` 失败。
